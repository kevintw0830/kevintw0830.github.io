...
const TARGET_COLORS = ["red", "green", "blue", "yellow", "cyan", "magenta"];
...
const targetQueueEl = document.querySelector("#target-queue");
...
function updateStats() {
  ...
  const targetColor = targetQueue[0] || "yellow";

  scoreEl.textContent = score.toString();
  goalEl.textContent = `先消${COLOR_LABELS[targetColor]}`;
  goalStatEl.classList.toggle("reached", targetMatched);
  goalStatEl.classList.toggle("missed", targetMissed);
  goalStatEl.style.setProperty("--goal-color", `var(--${targetColor})`);
  goalMeterEl.style.setProperty("--goal-fill", `${((matchedTargets % TARGET_QUEUE_LENGTH) / TARGET_QUEUE_LENGTH) * 100}%`);
  renderTargetQueue();
...
function createTargetQueue() {
  ...
  while (queue.length < TARGET_QUEUE_LENGTH) {
    queue.push(randomTargetColor(queue[queue.length - 1]));
  }

  return queue;
}

function randomTargetColor(previousColor = null) {
  const options = TARGET_COLORS.filter((color) => color !== previousColor);
  return options[Math.floor(Math.random() * options.length)];
}

function renderTargetQueue() {
  targetQueueEl.innerHTML = "";

  targetQueue.forEach((color, index) => {
    const chip = document.createElement("span");
    chip.className = `target-chip ${color}`;
    if (index === 0) chip.classList.add("current");
    chip.setAttribute("aria-label", `${COLOR_LABELS[color]}色目標`);
    targetQueueEl.appendChild(chip);
  });
}

function trackTargetQueue(clearing) {
  const clearedColors = new Set(clearing.map((cell) => board[cell.row][cell.col]));
  let matches = 0;

  while (targetQueue.length && clearedColors.has(targetQueue[0])) {
    const previousColor = targetQueue[targetQueue.length - 1];
    targetQueue.shift();
    targetQueue.push(randomTargetColor(previousColor));
    matches += 1;
    matchedTargets += 1;
  }
...
function pulseTarget(type) {
  targetMatched = type === "matched";
  targetMissed = type === "missed";
